...
export async function command(context: vscode.ExtensionContext) {
  ...
  if (roots.length > 0) {
    ...
    // TODO Ask for component name
    // -> Create folder with two files
    // -> Write in this two files
    const componentName = await showInputBox(dir);
    if (!componentName) return;

    createComponentFiles(componentName);
    // createFileOrFolder(componentName);
    // await openFile(componentName);
  } else {
